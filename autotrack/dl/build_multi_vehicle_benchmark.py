"""Build a multi-vehicle trajectory-energy benchmark.

This benchmark is the replacement for the old single-vehicle-only smoke path.
It uses the existing synthetic trajectory generator, but keeps all instances in
each window and stores a dense union energy target for multi-track training.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a multi-vehicle trajectory-energy benchmark.")
    ...
    parser.add_argument("--vehicles-max", type=int, default=20, help="Maximum vehicles per window.")
    ...
    parser.add_argument("--noise-std", type=float, default=0.0, help="Background noise standard deviation.")
    parser.add_argument("--amp-min", type=float, default=6.0, help="Minimum pulse amplitude.")
    parser.add_argument("--amp-max", type=float, default=6.0, help="Maximum pulse amplitude.")
    parser.add_argument("--sigma-min-s", type=float, default=0.25, help="Minimum pulse width in seconds.")
    parser.add_argument("--sigma-max-s", type=float, default=0.25, help="Maximum pulse width in seconds.")
    parser.add_argument("--min-visible-channels", type=int, default=3, help="Minimum visible channels per vehicle.")
    ...
    parser.add_argument("--artifact-decoy-amp-scale-min", type=float, default=1.05, help="Minimum decoy amplitude scale relative to the true track.")
    parser.add_argument("--artifact-decoy-amp-scale-max", type=float, default=1.8, help="Maximum decoy amplitude scale relative to the true track.")
    ...
    parser.add_argument("--artifact-competing-time-jitter-s", type=float, default=1.0, help="Random time jitter for the competing vehicle anchor.")
    parser.add_argument("--artifact-competing-amp-scale-min", type=float, default=0.8, help="Minimum competing-vehicle amplitude scale relative to the target track.")
    parser.add_argument("--artifact-competing-amp-scale-max", type=float, default=1.5, help="Maximum competing-vehicle amplitude scale relative to the target track.")
    parser.add_argument("--artifact-competing-speed-ratio-min", type=float, default=0.9, help="Minimum competing-vehicle speed ratio relative to the target track.")
    parser.add_argument("--artifact-competing-speed-ratio-max", type=float, default=1.1, help="Maximum competing-vehicle speed ratio relative to the target track.")
    parser.add_argument("--artifact-competing-channel-offset-max", type=int, default=4, help="Maximum channel offset for the competing vehicle anchor relative to the target anchor.")
    parser.add_argument("--artifact-competing-opposite-direction-ratio", type=float, default=0.25, help="Probability of assigning the competing vehicle the opposite direction.")
    ...
    parser.add_argument("--mask-sigma-ch", type=float, default=0.7, help="Target heatmap sigma in channel units.")
    parser.add_argument("--mask-sigma-t", type=float, default=1.6, help="Target heatmap sigma in downsampled time bins.")
...
def _energy_from_masks(gt_masks: torch.Tensor, *, n_channels: int, time_bins: int) -> torch.Tensor:
    if gt_masks.ndim != 3:
        raise ValueError(f"gt_masks must have shape [tracks, channels, time_bins], got {tuple(gt_masks.shape)}")
    if gt_masks.numel() == 0:
        return torch.zeros((int(n_channels), int(time_bins)), dtype=torch.float32)
    return gt_masks.to(torch.float32).amax(dim=0).clamp(0.0, 1.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.out_file.exists():
        if not bool(args.overwrite):
            raise FileExistsError(f"Output file already exists: {args.out_file}. Use --overwrite to replace it.")
    ...
    for idx in range(len(ds)):
        ...
        if gt_masks is None:
            raise ValueError("online synthetic dataset did not return gt_masks")
        energy = _energy_from_masks(gt_masks, n_channels=int(gt_masks.shape[1]), time_bins=int(gt_masks.shape[2]))
        union_visibility = gt_masks.to(torch.float32).amax(dim=0).amax(dim=-1)
        samples.append(
            {
                ...
                "energy": energy.cpu(),
                "union_visibility": union_visibility.cpu(),
            }
        )
    payload = {
        "format": "multi_vehicle_benchmark_v1",
