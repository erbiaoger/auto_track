...
class MethodRunner:
    def __init__(self, payload: dict[str, Any]):
        ...
        self.raw, self.pre, self.gauss = (np.load(str(src[k]), mmap_mode="r") for k in ("raw_path", "pre_path", "gauss_path"))
        self.positions, self.station_ids = _mapping(payload["mapping_path"]); self.device = str(payload.get("device", "cpu")); self.model = None
    ...
    def predict(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.method_id == "peak_slot":
            ...
            from autotrack.dl.vehicle_peak_set_transformer import PeakSetInferenceConfig, decode_peak_guided_vehicle_tracks, decode_vehicle_peak_tracks
            ...
            from autotrack.core.track_extractor_graph import ExtractorConfig, Track, TrackPoint
            cfg = ExtractorConfig(prominence=.4, min_peak_distance=500, min_track_channels=12, edge_min_track_channels=4, max_tracks=256)
            # The existing GPU entry point searches a channel-increasing
            # forward graph. DAY11 vehicles travel in the opposite physical
            # direction, so adapt only the channel order at the boundary and
            # map points back to the original physical mapping afterwards.
            graph_input = gauss.T[::-1]
            with contextlib.redirect_stdout(sys.stderr): decoded = extract_all_gpu(graph_input, fs=self.fs, dx_m=float(np.median(np.diff(self.positions))), direction="forward", vmin_kmh=60., vmax_kmh=90., config=cfg)
            mapped = []
            for item in decoded:
                points = []
            ...
            diagnostics = {"extractor": "classic_peak_graph_gpu", "track_count": len(tracks), "engine": "cupy_gpu", "direction": "reverse", "adapter": "reverse_channel_order"}
