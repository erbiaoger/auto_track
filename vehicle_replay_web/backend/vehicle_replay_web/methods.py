...
def _repo_root() -> Path:
    # vehicle_replay_web/backend/vehicle_replay_web/methods.py -> workspace root
    ...
    for raw_track in payload.get("tracks", []):
        points = [
            WorkerPoint(
                ...
                observation_id=int(item["observation_id"]) if item.get("observation_id") is not None else None,
                residual_s=float(item["residual_s"]) if item.get("residual_s") is not None else None,
...
class MethodWorker:
    def __init__(self, spec: dict[str, Any], *, root: Path, source: Any, mapping_path: str | Path, device: str, separation_config: dict[str, Any] | None = None, hybrid_config: dict[str, Any] | None = None) -> None:
        self.spec = spec
        self.root = root
        ...
        self.device = str(spec.get("device", device))
        self.separation_config = dict(separation_config or {"enabled": False})
        self.hybrid_config = dict(hybrid_config or {})
        input_kind = str(spec.get("input", ""))
        if input_kind == "raw_pre_gauss" and (source.raw_path is None or source.pre_path is None):
            raise WorkerError(f"{self.method_id} requires Raw + Pre + Gauss input")
        command = [str(interpreter), "-m", str(spec.get("worker_module", "hybrid_vehicle_tracker.worker")), "--method", self.method_id]
        ...
        init = {
            ...
            "source": {
                "raw_path": str(Path(source.raw_path).resolve()) if source.raw_path is not None else "",
                "pre_path": str(Path(source.pre_path).resolve()) if source.pre_path is not None else "",
                "prediction_path": str(Path(source.prediction_path).resolve()) if getattr(source, "prediction_path", None) is not None else "",
                "gauss_path": str(Path(source.gauss_path).resolve()),
                "sample_rate_hz": float(source.sample_rate_hz),
            },
            "separation": self.separation_config,
...
class MethodManager:
    def __init__(self, *, source: Any, mapping_path: str | Path, device: str, registry_path: str | Path | None = None) -> None:
        ...
        self.device = str(device)
        path = Path(registry_path) if registry_path else self.root / "vehicle_replay_web/backend/methods.yaml"
        if not path.is_absolute():
            path = (self.root / path).resolve()
        payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        self.stitch_config: dict[str, Any] = dict(payload.get("stitching", {}) or {})
        self.specs: dict[str, dict[str, Any]] = {}
        for raw in payload.get("methods", []):
            spec = dict(raw)
            spec["worker_module"] = spec.get("worker_module", payload.get("worker_module", "hybrid_vehicle_tracker.worker"))
        ...
        self.active_separation_config: dict[str, Any] = {}
        self.active_hybrid_config: dict[str, Any] = {}
        self.last_errors: dict[str, str] = {}

    def hybrid_config_payload(self) -> dict[str, Any]:
        """Return the YAML Hybrid parameters exposed by the web UI."""
        spec = self.specs.get("hybrid")
        if not spec or not spec.get("config"):
            return {}
        path = _resolve(self.root, str(spec["config"]))
        if not path.exists():
            return {}
        payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        # Paths and checkpoint names describe the selected dataset/model and
        # are shown read-only. Numeric and direction/seed values are editable.
        return payload
    ...
    def descriptors(self) -> list[dict[str, Any]]:
        ...
        for method_id, spec in self.specs.items():
            ...
            rows.append({
                ...
                "engine": spec.get("engine", self.device),
                "device": str(spec.get("device", self.device)),
    ...
    def _availability(self, spec: dict[str, Any]) -> tuple[bool, str | None]:
        if str(spec.get("input", "")) == "raw_pre_gauss" and (
            getattr(self.source, "raw_path", None) is None or getattr(self.source, "pre_path", None) is None
        ):
            return False, "当前数据只有 Gauss，缺少 Raw/Pre"
    ...
    def ensure(self, method_id: str | None = None, separation_config: dict[str, Any] | None = None, hybrid_config: dict[str, Any] | None = None) -> MethodWorker:
        selected = str(method_id or self.active_method_id or self.default_method)
        requested_separation = dict(separation_config) if separation_config is not None else {"enabled": False}
        requested_hybrid_config = dict(hybrid_config) if hybrid_config is not None else {}
        if selected not in self.specs:
            raise WorkerError(f"unknown method: {selected}")
        if self.active is not None and self.active_method_id == selected and self.active_separation_config == requested_separation and self.active_hybrid_config == requested_hybrid_config:
            return self.active
        ...
        old = self.active
        try:
            worker = MethodWorker(spec, root=self.root, source=self.source, mapping_path=self.mapping_path, device=self.device, separation_config=requested_separation, hybrid_config=requested_hybrid_config)
        except Exception as exc:
            self.last_errors[selected] = str(exc)
            raise
        self.active = worker
        self.active_method_id = selected
        self.active_separation_config = requested_separation
        self.active_hybrid_config = requested_hybrid_config
    ...
    def predict(self, *, start_s: float, duration_s: float) -> dict[str, Any]:
        worker = self.ensure(separation_config=self.active_separation_config, hybrid_config=self.active_hybrid_config)
        try:
    ...
    def close(self) -> None:
        ...
        self.active = None
        self.active_method_id = ""
        self.active_separation_config = {}
        self.active_hybrid_config = {}
