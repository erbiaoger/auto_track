...
import numpy as np
from .separation import separate_large_vehicle_signal
...
class DataChunk:
    ...
    def duration_s(self) -> float:
        values = self.raw if self.raw is not None else self.gauss
        return float(values.shape[0] / self.sample_rate_hz)
...
class NpyReplaySource:
    """Memory-mapped replay cache; Raw/Pre are optional for Gauss-only inputs."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dataset_label = str(payload.get("dataset_label", payload.get("dataset_id", manifest_path.parent.name)))
        ...
        self.raw_path = resolve(payload["raw_path"]) if payload.get("raw_path") else None
        self.pre_path = resolve(payload["pre_path"]) if payload.get("pre_path") else None
        self.prediction_path = resolve(payload["prediction_path"]) if payload.get("prediction_path") else None
        self.gauss_path = resolve(payload["gauss_path"])
        self.raw = np.load(self.raw_path, mmap_mode="r") if self.raw_path is not None else None
        self.pre = np.load(self.pre_path, mmap_mode="r") if self.pre_path is not None else None
        self.prediction = np.load(self.prediction_path, mmap_mode="r") if self.prediction_path is not None else None
        self.gauss = np.load(self.gauss_path, mmap_mode="r")
        arrays = [array for array in (self.raw, self.pre, self.prediction, self.gauss) if array is not None]
        shapes = {array.shape for array in arrays}
        if len(shapes) != 1 or self.gauss.ndim != 2:
        ...
        if self.gauss.shape[0] < expected_samples:
            raise ValueError("cache duration exceeds array length")
        self.station_count = int(self.gauss.shape[1])

        self.has_raw = self.raw is not None
        self.has_pre = self.pre is not None
        self.has_prediction = self.prediction is not None
    ...
    def read(self, start_s: float, duration_s: float) -> DataChunk:
        return DataChunk(
            ...
            raw=_slice(self.raw, start_s, duration_s, self.sample_rate_hz) if self.raw is not None else None,
            pre=_slice(self.pre, start_s, duration_s, self.sample_rate_hz) if self.pre is not None else None,
            prediction=_slice(self.prediction, start_s, duration_s, self.sample_rate_hz) if self.prediction is not None else None,
            gauss=_slice(self.gauss, start_s, duration_s, self.sample_rate_hz),
        )

    def frame(self, start_s: float, duration_s: float, separation_config: dict[str, object] | None = None) -> dict[str, object]:
        ...
        raw_min = _pool(chunk.raw, bins, "min") if chunk.raw is not None else np.empty((0, self.station_count), dtype=np.float32)
        raw_max = _pool(chunk.raw, bins, "max") if chunk.raw is not None else np.empty((0, self.station_count), dtype=np.float32)
        waveform_bins = self.waveform_downsample
        raw_wave = _pool(chunk.raw, waveform_bins, "mean") if chunk.raw is not None else np.empty((0, self.station_count), dtype=np.float32)
        ...
        if separation_config and bool(separation_config.get("enabled", False)):
            prediction = chunk.prediction if chunk.prediction is not None else chunk.pre
            large, _ = separate_large_vehicle_signal(
                chunk.raw, prediction, chunk.gauss, chunk.sample_rate_hz, separation_config,
            )
            frame["large_gauss"] = _pool(large, bins, "max").astype(np.float32).ravel().tolist()
        return frame
