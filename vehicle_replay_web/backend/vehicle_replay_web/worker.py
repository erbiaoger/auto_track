...
def _diagnostic_summary(value: Any) -> dict[str, Any]:
    ...
    for key, item in value.items():
        ...
        if isinstance(item, (str, int, float, bool)) or item is None:
            result[key] = item
        elif key == "large_vehicle_separation" and isinstance(item, dict):
...
class MethodRunner:
    def __init__(self, init: dict[str, Any]) -> None:
        ...
        self.device = str(init.get("device", "cuda"))
        if self.device != "cuda":
        ...
        self.hybrid_config_overrides = dict(init.get("hybrid_config", {}))
        self.model: Any = None
    ...
    def _load_method(self) -> None:
        if self.method_id == "hybrid":
            ...
            for section_name, values in self.hybrid_config_overrides.items():
                section = getattr(self.config, str(section_name), None)
    ...
    def predict(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.method_id == "hybrid" and self.separation_config.get("enabled", False):
            # Hybrid consumes all three modalities.  Filtering only Gauss
            # would leave small-vehicle evidence in Raw/Pre, making the web
            # large-vehicle threshold appear ineffective in the final tracks.
            mask = np.clip(gauss / max(float(self.separation_config.get("amplitude_max", 1.0)), 1e-6), 0.0, 1.0)
            raw = raw * mask
            non_event_pre = np.max(pre, axis=0, keepdims=True)
            pre = pre * mask + non_event_pre * (1.0 - mask)
            separation_diagnostics["hybrid_modalities_filtered"] = True
        if self.method_id == "hybrid":
        ...
        diagnostics["large_vehicle_separation"] = separation_diagnostics
        return {
            "protocol": PROTOCOL,
            "event": "prediction",
