...
class SingleVehicleBenchmarkFromLabelsTest(unittest.TestCase):
    def test_builds_benchmark_from_label_project(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                sys.argv = [
                    "build_single_vehicle_benchmark_from_labels",
                    "--out-file",
                    str(out_file),
                    "--source-npy",
                    str(source),
            ...
            self.assertGreaterEqual(int(payload.get("length", 0)), 2)
